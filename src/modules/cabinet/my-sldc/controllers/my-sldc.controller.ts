...
export async function createMySldc(req, res: Response) {
  ...
  const mySldcToDB = {
    ...
    archive_file0: linksArray[3] || '',
  }

  try {
    const newItem = await db('sldc').insert(mySldcToDB).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-sldc.controller', error)
