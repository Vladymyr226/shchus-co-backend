...
export async function createMyConstructorStartup(req, res: Response) {
  ...
  const myConstructorStartupToDB = {
  ...
  try {
    const newItem = await db('constructor_startups').insert(myConstructorStartupToDB).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-constructor-startup.controller', error)
