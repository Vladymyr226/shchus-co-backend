...
export async function createMySlFood(req, res: Response) {
  ...
  try {
    const newItem = await db('sl_food').insert(mySlFoodToDb).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-sl-food.controller', error)
