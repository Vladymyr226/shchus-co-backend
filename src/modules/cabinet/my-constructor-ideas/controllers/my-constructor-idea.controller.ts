...
export async function createMyConstructorIdea(req, res: Response) {
  ...
  for (let index = 0; index < arrDifferentTypesFiles.length; index++) {
    ...
    try {
      ...
      console.log('Error in my-constructor-idea.controller: ', error)
    }
  }
  const myConstructorIdeaToDB = {
  ...
  try {
    const newItem = await db('constructor_ideas').insert(myConstructorIdeaToDB).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-constructor-idea.controller', error)
