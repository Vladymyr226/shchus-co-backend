...
export async function createMySlBrain(req, res: Response) {
  ...
  try {
    const newItem = await db('sl_brain').insert(slBrainToDb).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-sl-brain.controller', error)
