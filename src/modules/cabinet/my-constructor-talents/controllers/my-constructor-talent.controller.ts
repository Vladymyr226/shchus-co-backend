...
export async function createMyConstructorTalent(req, res: Response) {
  ...
  for (let index = 0; index < arrDifferentTypesFiles.length; index++) {
    ...
    try {
      ...
      console.log('Error in my-constructor-talent.controller: ', error)
    }
  }
  ...
  try {
    const newItem = await db('constructor_talents').insert(myConstructorDesignerToDB).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-constructor-talent.controller', error)
