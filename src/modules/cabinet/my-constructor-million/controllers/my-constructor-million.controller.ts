...
export async function createMyConstructorMillion(req, res: Response) {
  ...
  const myConstructorMillionToDB = {
  ...
  try {
    const newItem = await db('constructor_million').insert(myConstructorMillionToDB).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-constructor-million.controller', error)
