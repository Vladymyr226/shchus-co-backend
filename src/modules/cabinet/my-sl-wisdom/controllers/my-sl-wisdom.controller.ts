...
export async function createMySlWisdom(req, res: Response) {
  ...
  try {
    const newItem = await db('sl_wisdom').insert(mySlWisdomToDB).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-sl-wisdom.controller', error)
