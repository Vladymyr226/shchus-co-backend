...
export async function createMySlSport(req, res: Response) {
  ...
  try {
    const newItem = await db('sl_sport').insert(slSportToDb).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-sl-sport.controller', error)
