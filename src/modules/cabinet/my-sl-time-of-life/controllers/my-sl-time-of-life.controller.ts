...
export async function createMySlTimeOfLife(req, res: Response) {
  ...
  try {
    const newItem = await db('sl_time_of_life').insert(slTimeOfLifeToDb).returning('*')
    console.log(newItem)

    return res.status(201).json({ message: 'OK' })
  } catch (error) {
    console.error('Error in my-sl-time-of-life.controller', error)
